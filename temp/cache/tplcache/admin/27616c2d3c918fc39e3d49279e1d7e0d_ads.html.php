...
<div id="admin_right_b">
  ...
  <table border="0" align="center" cellpadding="3" cellspacing="0" class="table_b">
	<tr>
      ...
      <td class="title_bg" width="100">标识(唯一)</td>
	  <td class="title_bg" width="150">模板调用标签</td>
	   <td class="title_bg" width="100">所属分组</td>
	   <td class="title_bg" width="150">终端(<font color="red">new</font>)</td>
	  <td class="title_bg" width="180">广告说明</td>
	  <td align='center' class="title_bg" width="120">自动插入位置</td>
      <td class="title_bg" width="60">屏蔽蜘蛛</td>
...
?>
    <tr onmouseover=this.bgColor='#EDF8FE'; onmouseout=this.bgColor='#ffffff'; bgcolor='#ffffff'>
	  ...
	  <td align="center" style="color:#ff6600"><?php echo (!isset($this->_var['vo']['terminal_name']) || $this->_var['vo']['terminal_name']==='') ? '全部终端' : $this->_var['vo']['terminal_name']; ?></td>
	  ...
	  <td align='center'>
		...
		<?php if($this->_var['vo']['insert_position'] == '5'): ?>插入&lt;/head&gt;前面<?php endif; ?>
		<?php if($this->_var['vo']['insert_position'] == '6'): ?>插入&lt;body&gt;后面<?php endif; ?>
	  </td>
      ...
      <td align="center"><a href="?admin-ads-preview-id-<?php echo $this->_var['vo']['id']; ?>" target='_blank' class="button button_small button_inverse">预览</a> <a href="?admin-ads-edit-id-<?php echo $this->_var['vo']['id']; ?>" class="button button_small"><i class="typcn typcn-edit"></i>修改</a> <a href="?admin-ads-del-id-<?php echo $this->_var['vo']['id']; ?>" onclick='return confirm("确定删除?删除后不可恢复!");' class="button button_small button_remove"><i class="typcn typcn-trash"></i></a></td>
	  <td>&nbsp;</td>
    </tr>
<?php endforeach; else: ?>
	<tr bgcolor='#ffffff'>
		<td colspan='10' height="25" align="center">暂无广告！</td>
	</tr>
<?php endif; unset($_from); ?><?php $this->pop_vars(); ?>

  </table>
<div class="divtips" style="margin-top:10px;">
<p>1.使用自动插入无需修改模板，如自动插入的位置不满意可手动使用调用标签写入模板中</p>
</div>
