  ...
  <table border="0" align="center" cellpadding="3" cellspacing="0" class="table_b">
   <form action="<?php echo url('admin/arctype/uporder'); ?>" method="post"> 
	<tr>
	  ...
	  <td width="100" align='center' class="title_bg">可取值参数</td>
	  <td width="100" align='center' class="title_bg">附加字段</td>
	  <td width="60" align='center' class="title_bg">域名数</td>
	  <td width="60" align='center' class="title_bg">栏目数</td>
	  <td width="60" align='center' class="title_bg">模板数</td>
      <td width="200" align='center' class="title_bg">管理</td>
...
?>
	<tr onmouseover=this.bgColor='#EDF8FE'; onmouseout=this.bgColor='#ffffff'; bgcolor='#ffffff'>
	  ...
	  <td style="overflow: hidden;max-width: 500px;text-overflow:ellipsis">&nbsp;<?php echo (!isset($this->_var['vo']['tplfiles']) || $this->_var['vo']['tplfiles']==='') ? 'index,list,show' : $this->_var['vo']['tplfiles']; ?></td>
	  <td align="center"><?php echo (!isset($this->_var['vo']['params']) || $this->_var['vo']['params']==='') ? '无' : $this->_var['vo']['params']; ?></td>
	  <td align="center"><?php echo (!isset($this->_var['vo']['addfields']) || $this->_var['vo']['addfields']==='') ? '无' : $this->_var['vo']['addfields']; ?></td>
	  <td align="center"><b style="color:green"><?php echo $this->_var['vo']['domain_num']; ?></b></td>
	  <td align="center"><b style="color:red"><?php echo $this->_var['vo']['catelog_num']; ?></b></td>
	  <td align="center"><b style="color:blue"><?php echo $this->_var['vo']['theme_num']; ?></b></td>
	  <td><a href="?admin-arctype-edit-id-<?php echo $this->_var['vo']['id']; ?>" class="button button_small" title="修改规则"><i class="typcn typcn-edit"></i>修改</a> <a href="javascript:" onclick="nodeImport('<?php echo $this->_var['vo']['id']; ?>');" class="button button_small button_inverse" title="导入模型">导入</a> <a href="javascript:" onclick="typeExport('<?php echo $this->_var['vo']['id']; ?>','<?php echo $this->_var['vo']['name']; ?>');" class="button button_small button_grey" title="导出模型">导出</a> <?php if($this->_var['vo']['system']): ?><a class='button button_small button_grey disabled' disabled><i class="typcn typcn-trash" style="color:#ddd"></i></a><?php else: ?><a href="?admin-arctype-del-id-<?php echo $this->_var['vo']['id']; ?>" onclick='return confirm("确定删除？删除后不可恢复！\n---------------------------\n注：模型下的栏目和内容库数据也会被删除！");' class="button button_small button_remove"><i class="typcn typcn-trash"></i></a><?php endif; ?></td>
	  <td align="center">&nbsp;</td>
	  </tr>
...
<p>温馨提示：</p>
<p>· 网站模型，用于区分网站内容（每个模型一个内容库）、风格模板、采集、URL规则模板等</p>
<p>· 导出的模型中包含了模型信息、URL模板规则、TKD调用模板</p>
</div>
