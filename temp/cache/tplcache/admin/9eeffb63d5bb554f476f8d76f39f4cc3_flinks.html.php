...
?>
	<dl class="default">
        <dt>
			...
			<p>友链条数：<b style="color:red"><?php echo (!isset($this->_var['vo']['flinks_num']) || $this->_var['vo']['flinks_num']==='') ? 0 : $this->_var['vo']['flinks_num']; ?></b> 条</p>
			<p>-----调用域名------<br><?php echo (!isset($this->_var['vo']['domain']) || $this->_var['vo']['domain']==='') ? '未设置' : $this->_var['vo']['domain']; ?>...<br>-----------</p>
			<p>修改时间：<b><?php echo now_date_color($this->_var['vo']['addtime']); ?></b></p>
		</dt>
        <dd><a class="button button_small button_success" href="javascript:" onclick="edit('<?php echo $this->_var['vo']['id']; ?>','<?php echo $this->_var['vo']['name']; ?>');"><i class="typcn typcn-edit"></i>修改</a>&nbsp;&nbsp;<a href="?admin-flinks-del-id-<?php echo $this->_var['vo']['id']; ?>" class="button button_small button_remove" onclick='return confirm("确定删除?删除后不可恢复!");'><i class="typcn typcn-trash"></i></a></dd>
      </dl>
...
<tr>
      <td colspan="5" class="tdbg">
		<div class="divtips">
			<p>如果模板并未添加有友情链接的调用，请修改模板加入调用标签。</p>
		</div>
		<div>
			<p>模板调用标签如下：<br>
...
function edit(id,typename){
	var title=id?'修改':'添加';
	edit_dialog({
		title:title+'友情链接配置【'+typename+'】',
		url:'?admin-flinks-edit-id-'+id,
		posturl:'?admin-flinks-update',
		width:800,
		height:520
	});
}
