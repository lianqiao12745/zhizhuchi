...
<tr class="tdbg">
  ...
  <td class="icheck_radios"><label><input type="radio" name="con[saveapi_open]" value="1" <?php if($this->_var['saveapi_open']): ?> checked<?php endif; ?>>开启</label>
		<label><input type="radio" name="con[saveapi_open]" value="0" <?php if(! $this->_var['saveapi_open']): ?> checked<?php endif; ?>>关闭</label> <span class="tips"> 接口开关</span>
	</td>
</tr>
<tr>
  <td align="right" class="config_td_first">密钥：</td>
  <td><input type="text" style="text-align:left" name="con[saveapi_sign]" class="input" value="<?php echo $this->_var['saveapi_sign']; ?>" size="20"> <span class="tips">设置接口密钥，数字和字母组成</span></td>
</tr>

<tr class="tdbg">
	<td align="right" class="config_td_first">内容过滤标签：</td>
	<td class="icheck_radios">
		<label><input type="checkbox" id="taglist_all" name="taglist[]" value="all" <?php if($this->_var['saveapi_sift_taglists'] == 'all'): ?> checked<?php endif; ?> /><font color="#ff6600">使用内置过滤（过滤大部分无用标签，包括A标签）</font></label><hr style="border: 0;border-bottom: 1px dotted #ccc;">
...
?>
			<label><input type="checkbox" class="taglist_item" name="taglist[]" value="<?php echo $this->_var['vo']; ?>" <?php if($this->_var['saveapi_sift_taglists'] == 'all'): ?> disabled<?php endif; ?> <?php if($this->_var['saveapi_sift_taglists'] && in_array ( $this->_var['vo'] , $this->_var['saveapi_sift_taglists'] )): ?> checked<?php endif; ?> /><?php echo $this->_var['vo']; ?></label>
...
$("#save_sign").click(function(){
	...
	$.ajax({
		type: "POST",
		url: '<?php echo url('admin/collect/saveapi_update'); ?>',
		data:$("form").serialize(),
		success: function (d) {
			if(d.status==1){
...
$("#ssbtn").click(function(){
	...
	$.ajax({
		type: "POST",
		dataType: "html",
		url: '<?php echo url('admin/collect/saveapi_makeurl'); ?>',
