		...
		<tbody>
			...
			<tr class="tdbg">
			  ...
			  <td class="icheck_radios"><label><input type="radio" id="dbtype1" name="dbtype" value="1" checked>模型公共库</label>
					...
					<?php if($this->_var['input_name'] == 'article'): ?><label><input type="radio" id="dbtype3" name="dbtype" value="3">栏目文章库</label><?php endif; ?>
				</td>
			</tr>
			<tr id="module_tr">
...
upload('#up<?php echo $this->_var['input_name']; ?>','txt',null,'<?php echo $this->_var['input_name']; ?>/'+$('#arctype').val()+'|'+dbtype+'|'+article_type);
...
function change_type(article_type){
	...
	$('#updiv').html(newhtml);
	upload('#up<?php echo $this->_var['input_name']; ?>'+i,'txt',null,'<?php echo $this->_var['input_name']; ?>/'+dirname+'|'+dbtype+'|'+article_type);
	i++;
}
</script>