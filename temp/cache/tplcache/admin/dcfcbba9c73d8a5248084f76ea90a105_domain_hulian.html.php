...
<div class="divtips">
	注，程序执行流程：获取《加入链接的网站分组》的链接地址，在《展示的网站分组》的页面上显示。
</div>
...
<table width="98%" border="0" align="center" cellpadding="4" cellspacing="0" id="config0" class="tableConfig">
	<tbody>
		...
		<tr class="tdbg">
		  ...
		  <td class="tdbg"><input name="con[domain_default_num]" type="text" class="input" value="<?php echo (!isset($this->_var['domain_default_num']) || $this->_var['domain_default_num']==='') ? '10' : $this->_var['domain_default_num']; ?>" size="10"></td>
		</tr>
		<td align="right" class="config_td_first">加入链接的网站分组：</td>
			<td><input type="hidden" name="con[addlink_group]" id="addlink_group_value" value="<?php echo $this->_var['addlink_group']; ?>" /><font id="addlink_group_items" color="#ff6600"><?php if(! $this->_var['addlink_group_name']): ?>未选择<?php else: ?><?php echo $this->_var['addlink_group_name']; ?><?php endif; ?></font>&nbsp;&nbsp;<a href="javascript:" class="button button_small" onclick="select_group('addlink');"><i class="typcn typcn-zoom-in"></i>点击选择</a> <span class="tips" style="color:red"><i class="typcn typcn-info"></i>选择加入链接的网站分组</span></td>
		</tr>
		<tr>
			<td align="right" class="config_td_first">展示的网站分组：</td>
			<td><input type="hidden" name="con[hulian_group]" id="hulian_group_value" value="<?php echo $this->_var['hulian_group']; ?>" /><font id="hulian_group_items" color="#ff6600"><?php if(! $this->_var['hulian_group_name']): ?>未选择<?php else: ?><?php echo $this->_var['hulian_group_name']; ?><?php endif; ?></font>&nbsp;&nbsp;<a href="javascript:" class="button button_small" onclick="select_group('hulian');"><i class="typcn typcn-zoom-in"></i>点击选择</a> <span class="tips" style="color:red"><i class="typcn typcn-info"></i>选择展示链接的网站分组</span></td>
		</tr>
		<tr class="tdbg">
		...
		<tr>
			<td width="120" align="right" class="config_td_first">单域名模式自动加www：</td>
			<td class="icheck_radios tdbg"><label><input type="radio" name="con[hulian_www]" value="1" <?php if($this->_var['hulian_www'] == '1' || $this->_var['hulian_www'] == ''): ?> checked<?php endif; ?>>是</label>
			<label><input type="radio" name="con[hulian_www]" value="0" <?php if($this->_var['hulian_www'] == '0'): ?> checked<?php endif; ?>>否</label>
			<span>单域名模式时，域名是否自动加www</span>
			</td>
		</tr>
		<tr class="tdbg">
		  <td align="right" class="config_td_first">互链描文本：</td>
		  <td class="tdbg"><select name="con[domain_title]" onchange="domain_title(this.value);">
					<option value="title" <?php if(!$this->_var['domain_title']||$this->_var['domain_title'] == 'title'): ?> selected="selected"<?php endif; ?>>文章标题</option>
