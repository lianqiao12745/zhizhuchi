(function () {
   ...
   // }
    ...
    var REWARD_PLUGIN_STR = {
        UNHAPPY: "???????????????????????????????",
        DEFAULT: "?????????????????????~",
        ORDER_URL: bloghost+"zb_users/plugin/reward/alipay/api.php?act=order",
        QUERY_URL: bloghost+"zb_users/plugin/reward/alipay/api.php?act=query",
    ...
    function onWaitBuyerPay(qrpay_query_response){
        setRewardText("???????????????" + qrpay_query_response.buyer_logon_id + "?????????????????????");
    }
    function onTradeSuccess(qrpay_query_response){
        setRewardText("?????????????????????" + qrpay_query_response.buyer_pay_amount + "?????????????");
    ...
    $(REWARD_PLUGIN_STR.RANDOM_AMOUNT_SELECTOR).click(function () {
        $(REWARD_PLUGIN_STR.TOTALAMOUNT_SELECTOR).scroll();
