...
(function(window, undefined){
	...
	TouchSlider.fn=TouchSlider.prototype={
		...
		_default: {
			'id':'slider', //õid
			'begin':0,
			'auto':true, //ǷԶʼʾԶʼ0,1,2,3....ʾԶʼԼӵڼʼ
			'speed':600, //Чʱ ms
			'timeout':5000,//õƼʱ ms,
		...
		_move:function(evt){
			...
			if(this.scrolling || typeof this.scrolling=='undefined'&&Math.abs(offset)>=Math.abs(this.stopPos[1-this.vertical]-this.startPos[1-this.vertical])){
				...
				if(offset&&typeof this.scrolling=='undefined'){
					this.scrolling=true;//϶Ч
					clearTimeout(this.timer);//ͣõ
					clearTimeout(this.aniTimer);//ͣ
				}
...
TouchScroll.prototype={
	...
	/*û߻ȡcss
	 *@param HTMLElement el: ҪĽڵ
	 *@param Object css: ʽ
	 *@param string css: ڶcssַΪʽĹؼ֣򷵻ضӦʽֵ
	 */
	css:(function(){
	...
	/*ӿڣЧ
	 *@param Number offx: ˮƽϹoffx
	 *@param Number offy: ֱϹoffy
	 */
	...
	/*ṩЧ
	 *@param Number flag: 10,0ˮƽ1ֱ
	 *@param Number distance: ƶľ
	 *@param Number during: ʱ
	 *@param String fx: Ч
	 *@param Function callback: ɺĻص
	 */
	slide:function(flag,distance,during,fx,callback){
		var _this=this, _callback=callback||new Function(),
			...
			_fx=fx||'ease-out';//Ч
		function animate(t,b,c,d,fx){ //Ч㹫ʽ
			var re;
			switch(fx){
				case 'ease-in-out':
	...
	start:function(e){
		clearTimeout(this.timer[0]); clearTimeout(this.timer[1]);//ڽеĶ
	...
	move:function(e){
		...
		if(this.target){//϶ǹҪ
			var barLength=parseInt(this.css(this[this.property[this.flag][6]],this.property[this.flag][2]));//õĳ
		...
		if(typeof this.flag === 'number'){
			...
			if(offset>0){
				...
				if(this.disc[0].length){
					...
					dist=rect[this.flag];//
					if(Math.abs(dist)>20){//ʱƶ20
						distance=(2-time/200)*dist;	//Ҫƶľ
					}
				}
			}
